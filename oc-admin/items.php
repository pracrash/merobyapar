        ...
        function doModel()
        {
            ...
            switch ($this->action)
            {
                ...
                case 'status_premium':  //status premium
                                        ...
                                        $this->redirectTo( $_SERVER['HTTP_REFERER'] );
                break;
                //added by shakeelstha for setting item as premium for carousel
                case 'status_premium_carousel':  //status premium carousel
                        $id = Params::getParam('id') ;
                        $value = Params::getParam('value') ;
                        ...
                        if ($mItems->premium_carousel($id, $value==1?true:false) ) {
                            osc_add_flash_ok_message( _m('Changes have been applied'), 'admin');
                        } else {
                            osc_add_flash_error_message( _m('Some error has occurred'), 'admin');
                        }
