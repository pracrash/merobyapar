     ...
     class ItemsProcessingAjax 
     {
        ...
        private function toArrayFormat() {
            ...
            foreach ($this->items as $aRow)
            {
                ...
                if( $aRow['b_premium_scroller'] ) {
                    $options_more[] = '<a href="' . osc_admin_base_url(true) . '?page=items&amp;action=status_premium_carousel&amp;id=' . $aRow['pk_i_id'] . '&amp;value=0">' . __('Unmark as carousel premium') .'</a>' ;
                } else {
                    $options_more[] = '<a href="' . osc_admin_base_url(true) . '?page=items&amp;action=status_premium_carousel&amp;id=' . $aRow['pk_i_id'] . '&amp;value=1">' . __('Mark as carousel premium') .'</a>' ;
                }
                if( $aRow['b_spam'] ) {
