...
function fbc_button() {
    ...
    if( $user && osc_is_web_user_logged_in() ) {
        echo '<a href="' . OSCFacebook::newInstance()->logoutUrl() . '">' . __( 'FB Logout', 'facebook' ) . '</a>' ;
    } else {
        echo '<a href="' . OSCFacebook::newInstance()->loginUrl() . '"><img src="' . osc_current_web_theme_url('images/fbconnect.png') . '"/></a>' ;
    }
}
...
osc_add_hook( 'header', 'fbc_header' ) ;

    /* file end: ./oc-content/plugins/facebook/index.php */

function fbc_header() {
    if($_GET["facebook_logout"]=='true') {
        session_start();
        session_unset();
        session_destroy();
        $_SESSION = array();
...
function fb_logout() {
    echo '<a href="' . osc_base_url() . '?facebook_logout=true' . '">' . __( 'Logout', 'facebook' ) . '</a>' ;
}
?>
