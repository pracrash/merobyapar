<?php
/*
Plugin Name: Personal Messaging
Plugin URI: 
Description: A Personal Messaging System
Version: 1.5.1
Author: JChapman
...
// Everything between this section is user side related.  
    ...
    */ ?>
       ...
       if(osclass_pm_is_pub_profile() && osc_is_web_user_logged_in()) {
          ...
          <script type="text/javascript" >
            $(document).ready(function(){
               $('#user_data').append("<p class=\"contact_button\"><strong class=\"share\"><a href=\"<?php echo osc_user_login_url() . '&http_referer=' . osc_base_url(true) . '?page=custom&file=osclass_pm/user-send.php&userId=' . $userId . '&mType=new'; ?>\"><?php echo __('Login to contact seller.','osclass_pm'); ?></a></strong></p>");
            });
          </script>
          <?php
       }
    ...
    function osclass_pm_user_menu() {      
       ...
       echo '<li class="inbox" ><a href="' . osc_render_file_url(osc_plugin_folder(__FILE__) . 'user-inbox.php') . '" ><i class="icon-envelop"></i> ' . __('Inbox', 'osclass_pm') . ' (' . $countPMs . ')</a></li>';
       if(pmDrafts()){
         echo '<li class="draft"><a href="' . osc_render_file_url(osc_plugin_folder(__FILE__) . 'user-drafts.php') . '" ><i class="icon-user"></i> ' . __('Drafts', 'osclass_pm') . ' (' . $countPMdrafts . ')</a></li>';
       }
       if(pmSent()) {
         echo '<li class="outbox" ><a href="' . osc_render_file_url(osc_plugin_folder(__FILE__) . 'user-outbox.php') . '" ><i class="icon-user"></i> ' . __('Outbox', 'osclass_pm') . '</a></li>' ;
       }
       echo '<li class="pmsetting"><a href="' . osc_render_file_url(osc_plugin_folder(__FILE__) . 'user-pm-settings.php') . '" ><i class="icon-user"></i> ' . __('PM Settings', 'osclass_pm') . '</a></li>' ;
    }
