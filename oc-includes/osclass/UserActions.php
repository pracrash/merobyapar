<?php if ( !defined('ABS_PATH') ) exit('ABS_PATH is not loaded. Direct access is not allowed.');
    ...
    Class UserActions
    {
        ...
        function edit($userId)
        {
            ...
            if ( is_array( Params::getParam('description') ) ) {
                foreach (Params::getParam('description') as $key => $value) {
                    $this->manager->updateDescription($userId, $key, $value) ;
                }
            }
            
            // For The Categories Selected
            if( is_array( Params::getParam('multiselect_sCategory') ) ) {
                $this->manager->deleteCategory($userId) ;
                foreach (Params::getParam('multiselect_sCategory') as $key => $value) {
                    $this->manager->updateCategory($userId, $value);
                }
            }
            ...
            if( isset( $_FILES['photos'] ) ) {
                ...
                //$pos = array_search('', $photos['name']);

                if ( !$this->checkAllowedExt($photos) ) {
                    $flash_error .= _m("Image with an incorrect extension.") . PHP_EOL;
                } else if ( !$this->checkSize($photos) ) {
                    $flash_error .= _m("Image is too big. Max. size") . osc_max_size_kb() ." Kb" . PHP_EOL;
                }
                $this->uploadCompanyPhotos($photos, $userId);
            }
            
            
            $this->manager->updateYouTube($userId, Params::getParam('youTube'));
        ...
        public function uploadCompanyPhotos($aResources,$userId)
        {
            if($aResources != '') {
                $wat = new Watermark();

                $userPhotosManager = User::newInstance() ;

                $numImagesItems = 5;
                $numImages = $userPhotosManager->countCompanyPhotos($userId);
                foreach ($aResources['error'] as $key => $error) {
                    if($numImagesItems==0 || ($numImagesItems>0 && $numImages<$numImagesItems)) {
                        if ($error == UPLOAD_ERR_OK) {
                            ...
                            if($this->manager->insertCompanyPhoto($userId)) {
                                $photoId = $userPhotosManager->dao->insertedId();

                                // Create normal size
                                $normal_path = $path = osc_content_path() . 'uploads/' . $photoId . '_company.jpg' ;
                                ...
                                // Create thumbnail
                                $path = osc_content_path(). 'uploads/' . $photoId . '_company_thumbnail.jpg' ;
                                ...
                                $this->manager->updateCompanyPhoto(
                                                        array(
                                                            's_path'            => $s_path
                                ...
                                osc_run_hook('uploaded_file', ItemResource::newInstance()->findByPrimaryKey($photoId));
                            }
                        }
                    }
                }
                unset($userPhotosManager);
            }
        }
    }
