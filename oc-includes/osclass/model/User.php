 ...
 */
    ...
    /**
     * Find an user by its user type value (0=individual, 1=company)
     *
     * @access public
     * @since unknown
     * @param string $email
    ...
    public function findByUserType($type_value, $alphabet) {
        $this->dao->select() ;
        $this->dao->from($this->getTableName()) ;
        $this->dao->like('s_name', $alphabet,'after') ;
        $this->dao->where('b_company', $type_value) ;

        //$this->dao->where('fk_i_category_id != 204 AND fk_i_category_id !=205');
        $this->dao->orderBy('s_name', "DESC") ;
    ...
    public function deleteUser($id = null) {
        if($id!=null) {
            ...
            $this->dao->delete(DB_TABLE_PREFIX.'t_user_company_youtube', array('fk_i_user_id' => $id)) ;
            $this->dao->delete(DB_TABLE_PREFIX.'t_user_sell_category', array('fk_i_user_id' => $id)) ;
            $this->dao->delete(DB_TABLE_PREFIX.'t_user_company_picture', array('fk_i_user_id' => $id)) ;
     ...
     * @return array
     */
    public function insertDescription($id, $locale, $info) {
        $array_set = array(
            'fk_i_user_id'      => $id,
     ...
     * @return array
     */
    public function insertCategory($id, $categoryId) {
        $array_set = array(
            'fk_i_user_id'      => $id,
            'fk_i_category_id'  => $categoryId
        );
        return $this->dao->insert(DB_TABLE_PREFIX.'t_user_sell_category', $array_set) ;
    }    
    
    /**
     * 
     * Updates User's selected categories
     * 
     * @param int $id
    ...
    public function updateCategory($id, $categoryId) {
        $this->insertCategory($id, $categoryId);
    }
    ...
    public function deleteCategory($id) {
        ...
        $this->dao->delete(DB_TABLE_PREFIX.'t_user_sell_category', $array_where);
    }
     ...
     * @return array
     */
    public function insertYouTube($id, $youtube) {
        $array_set = array(
            'fk_i_user_id'      => $id,
            's_youtube'  => $youtube
        );
        return $this->dao->insert(DB_TABLE_PREFIX.'t_user_company_youtube', $array_set) ;
    }    
    ...
    public function updateYouTube($id, $youtube) {
        $conditions = array('fk_i_user_id' => $id);
        $exist = $this->existYouTube($conditions);

        if(!$exist) {
            $result = $this->insertYouTube($id, $youtube);
            return $result;
        }

        return $this->dao->update(DB_TABLE_PREFIX.'t_user_company_youtube', array('s_youtube' => $youtube), $conditions) ;
    }
     ...
     * @return bool
     */
    private function existYouTube($conditions) {
        $this->dao->select() ;
    ...
    public function insertCompanyPhoto($id) {
        $array_set = array(
            'fk_i_user_id'  => $id
        );
        return $this->dao->insert(DB_TABLE_PREFIX.'t_user_company_picture', $array_set) ;
    }
    
    public function updateCompanyPhoto($set_values, $conditions) {

        return $this->dao->update(DB_TABLE_PREFIX.'t_user_company_picture', $set_values, $conditions) ;
    }
    ...
    public function deleteCompanyPhoto($id) {
        ...
        return $this->dao->delete(DB_TABLE_PREFIX.'t_user_company_picture', $conditions);
    }
    
    /**
    * Count resouces belong to Company Info given its id
    *
    * @access public
    * @since unknown
    ...
    * @return int 
    */
    function countCompanyPhotos($userId) {
        $this->dao->select('COUNT(*) AS numrows') ;
        $this->dao->from( DB_TABLE_PREFIX.'t_user_company_picture' ) ;
