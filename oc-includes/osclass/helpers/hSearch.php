<?php
    ...
    */
	...
	/**
     * Gets list of location around cities with items
     *
     * @return array
     */
	 //added by shakeelstha
    function osc_list_around_city() {
        if (View::newInstance()->_exists('list_around_cities')) {
            return View::newInstance()->_current('list_around_cities') ;
        } else {
            return null;
        }
    }
	...
	/**
     * Gets the next location around city in the list_cities list
     *
     * @param string $region
     * @return array
     */
	 //added by shakeelstha
    function osc_has_list_around_cities($region = '%%%%') {
        if ( !View::newInstance()->_exists('list_around_cities') ) {
            View::newInstance()->_exportVariableToView('list_around_cities', CityStats::newInstance()->listAroundCities($region) ) ;
        }
        $result = View::newInstance()->_next('list_around_cities');
        if (!$result) {
            View::newInstance()->_reset('list_around_cities') ;
        }
        return $result;
    }
     ...
     */
	 //added by shakeelstha
    function osc_count_list_around_cities($region = '%%%%') {
        if ( !View::newInstance()->_exists('list_around_cities') ) {
            View::newInstance()->_exportVariableToView('list_around_cities', CityStats::newInstance()->listAroundCities($region) ) ;
        }
        return View::newInstance()->_count('list_around_cities') ;
    }
	...
	// city attributes
    /**
     * Gets the the name of current "list location around city""
     *
     * @return string
     */
	 //added by shakeelstha
    function osc_list_around_city_name() {
        return osc_field(osc_list_around_city(), 'around_city_name', '') ;
    }
     ...
     */
	 //added by shakeelstha
    function osc_list_around_city_url() {
        if ( osc_rewrite_enabled() ) {
            $url = osc_base_url();
            if( osc_get_preference('seo_url_search_prefix') != '' ) {
                $url .= osc_get_preference('seo_url_search_prefix') . '/';
            ...
            //return osc_search_url(array('sCity' => osc_list_city_name()));
			return osc_search_url(array('sCity' => osc_list_around_city_name()));
        }
    }
